...
namespace ProjectCommon
{
	...
	public class GameEngineInitialization : EngineInitialization
	{
		...
		bool IsHDRSupported()
		{
			var compositor = CompositorManager.Instance.GetByName("HDR");
			if (compositor == null || !compositor.IsSupported())
				return false;

			var floatTexturesSupported = TextureManager.Instance.IsEquivalentFormatSupported(
		...
		public static void ConfigureMRTRendering()
		{
			    ...
			    RenderSystem.Instance.IsDirect3D())
			{
				...
				{
					formats = new[] {PixelFormat.Float32R}; // PixelFormat.Float16R
				}
				else
				{
					if (EngineApp.RenderTechnique == "HDR")
						formats = new[] {PixelFormat.Float16RGB};
					else
						formats = new[] {PixelFormat.Float16GR};
				}
